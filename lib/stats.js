  ...
  fs.readdir(projectDir)
    ...
    .then(entries => {
      ...
      entries.forEach(entry => {
        ...
        if (entry[1]) output.push('\t' + msg.MSG_FIRST_ADDED + ' ' + new Date(entry[1]).toLocaleString());
        if (entry[2]) output.push('\t' + msg.MSG_LAST_EDITED + ' ' + new Date(entry[2]).toLocaleString());
        output.push('\t' + msg.MSG_EDITED_COUNT + ' ' + entry[3]);
      })
