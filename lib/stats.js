  ...
  fs.readdir(projectDir)
    ...
    .then(entries => {
      return [
        ...
        entries.map(change => change.title),
    ...
    .then(entries => {
      ...
      entries.forEach(entry => {
        ...
        if (entry[2]) output.push('\t' + msg.MSG_FIRST_ADDED + ' ' + new Date(entry[2]).toLocaleString());
        if (entry[3]) output.push('\t' + msg.MSG_LAST_EDITED + ' ' + new Date(entry[3]).toLocaleString());
        output.push('\t' + msg.MSG_EDITED_COUNT + ' ' + entry[4]);
      })
